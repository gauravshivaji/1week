...
st.title("📊 Nifty500 Buy/Sell Predictor (Weekly) — Rules + Elliott Wave + ML")
...
NIFTY500_TICKERS = [
    ...
    "PAYTM.NS","OF
SS.NS","POLICYBZR.NS","PCBL.NS","PGEL.NS","PIIND.NS","PNBHOUSING.NS","PNCINFRA.NS","PTCIL.NS","PVRINOX.NS",
...
def download_data_multi(tickers, period="5y", interval="1wk"):
    ...
    for i in stqdm(
        ...
        desc="Downloading weekly data",
        total=len(tickers) // batch_size + 1,
    ):
...
def zigzag_pivots(close: pd.Series, pct=0.05, min_bars=5):
    """
    Dependency-free ZigZag: pivots when price reverses by >= pct from
    last pivot and min_bars elapsed.
    ...
    for i in range(1, len(c)):
        ...
        if trend in (None, "up"):
            ...
            if dd <= -pct and (i - last_piv_i) >= min_bars:
                ...
                last_piv_i = last_extreme_i
                last_piv_p = last_extreme_p
                trend = "down"
                last_extreme_i = i
                last_extreme_p = c[i]
...
def elliott_phase_from_pivots(pivots: pd.DataFrame):
    ...
    Returns dict:
      phase {'ImpulseUp','ImpulseDown','CorrectionUp','CorrectionDown','Unknown'},
      wave_no (5 or 3),
      bullish, bearish.
    """
...
def get_features_for_all(tickers, sma_windows, support_window, zz_pct, zz_min_bars):
    ...
    if isinstance(multi_df.columns, pd.MultiIndex):
        ...
        for ticker in tickers:
            ...
            if tdf.empty:
                continue
            feats = get_latest_features_for_ticker(
                tdf, ticker, sma_windows, support_window, zz_pct, zz_min_bars
            )
            if feats:
                features_list.append(feats)
    else:
        feats = get_latest_features_for_ticker(
            multi_df.dropna(), tickers[0], sma_windows, support_window, zz_pct, zz_min_bars
        )
        if feats:
            features_list.append(feats)
...
def predict_buy_sell_rule(df, rsi_buy=30, rsi_sell=70):
    ...
    ew_only_buy = ew_bull & results["RSI"].between(35, 65) & (results["Close"] > results["SMA20"])
    ew_only_sell = ew_bear & (results["RSI"] > 50)
...
def build_ml_dataset_for_tickers(
    ...
    rsi_buy=30,
    rsi_sell=70,
    min_rows=150,  # Adjusted for weekly data
    zz_pct=0.05,
    zz_min_bars=5,
):
    ...
    for t in stqdm(tickers, desc="Preparing ML data"):
        ...
        use = (
            ...
            .drop(columns=list(drop_cols & set(data.columns)), errors="ignore")
        )
...
with st.sidebar:
    ...
    sma_w1 = st.number_input("SMA Window 1 (weeks)", 5, 250, 20)
    sma_w2 = st.number_input("SMA Window 2 (weeks)", 5, 250, 50)
    sma_w3 = st.number_input("SMA Window 3 (weeks)", 5, 250, 200)
    support_window = st.number_input("Support Period (weeks)", 5, 200, 30)
    ...
    zz_pct = st.slider(
        ...
        12,
        5,
        help="Sensitivity for swing detection (5% = 0.05).",
    ) / 100.0
    zz_min_bars = st.slider(
        "Min bars between pivots",
        3,
        15,
        5,
        help="Minimum number of weeks between pivots.",
    )

    st.markdown("---")
    label_mode = st.radio(
        "ML Labeling Mode",
        ["Rule-based (teach the rules)", "Future Returns"],
        index=0,
    )
    ...
    if label_mode == "Rule-based (teach the rules)":
        ...
        ml_horizon, ml_buy_thr, ml_sell_thr = 8, 0.05, -0.05  # Default weekly values
    else:
        ...
        ml_horizon = st.number_input("Horizon (weeks ahead)", 1, 52, 8)
        ml_buy_thr = st.number_input(
            "Buy threshold (e.g., 0.05 = +5%)", 0.01, 0.50, 0.05, step=0.01, format="%.2f"
        )
        ml_sell_thr = st.number_input(
            "Sell threshold (e.g., -0.05 = -5%)",
            -0.50,
            -0.01,
            -0.05,
            step=0.01,
...
if st.session_state.analysis_run:
    ...
    with st.spinner("Fetching weekly data & computing rule-based + Elliott features..."):
    ...
    if feats is None or feats.empty:
        ...
        with tab1:
            if "preds_rule" not in locals() or preds_rule.empty:
                ...
                cols = [c for c in show_cols if c in df_buy.columns] + [
                    c for c in df_buy.columns if c not in show_cols
                ]

                st.write(df_buy[cols].to_html(escape=False, index=False), unsafe_allow_html=True)

        # ---- TAB 2: Rule-based Sell ----
        with tab2:
            if "preds_rule" not in locals() or preds_rule.empty:
                ...
                cols = [c for c in show_cols if c in df_sell.columns] + [
                    c for c in df_sell.columns if c not in show_cols
                ]
        ...
        with tab3:
            ...
            chart_df = yf.download(
                ticker_for_chart, period="3y", interval="1wk", progress=False, threads=True
            )

            if not chart_df.empty:
                chart_df = compute_features(
                    chart_df, sma_tuple, support_window, zz_pct, zz_min_bars
                ).dropna()
                ...
                if not chart_df.empty:
                    ...
                    st.caption(
                        ...
                        f"ZigZag: {zz_pct*100:.1f}% / {zz_min_bars} weeks"
                    )
                else:
        ...
        with tab4:
            if not SKLEARN_OK:
                ...
                with st.spinner("Building ML dataset & training model on weekly data..."):
                    if label_mode == "Rule-based (teach the rules)":
                ...
                if X.empty or y.empty:
                        ...
                        rows.append(
                            {
                                ...
                                "Prob_Buy": float(proba[list(clf.classes_).index(1)])
                                if proba is not None and 1 in clf.classes_
                                else np.nan,
                                "Prob_Hold": float(proba[list(clf.classes_).index(0)])
                                if proba is not None and 0 in clf.classes_
                                else np.nan,
                                "Prob_Sell": float(proba[list(clf.classes_).index(-1)])
                                if proba is not None and -1 in clf.classes_
                                else np.nan,
                            }
                        )
                    ...
                    if "ml_df" in locals() and "feats" in locals() and not feats.empty:
                        ...
                        st.download_button(
                            ...
                            file_name="ml_weekly_signals_with_price.csv",
                            mime="text/csv",
                        )
                    ...
                    if "preds_rule" in locals() and preds_rule is not None and not preds_rule.empty:
                        st.download_button(
                            ...
                            "nifty500_weekly_rule_signals.csv",
                            "text/csv",
                        )
