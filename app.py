...
with st.sidebar:
    ...
    zz_min_bars = st.slider("Min bars between pivots", 3, 15, 5, help="Minimum number of weeks between pivots.")
    ...
    if label_mode == "Rule-based (teach the rules)":
        ...
        ml_buy_thr = st.number_input("Buy threshold (e.g., 0.05 = +5%)", 0.01, 0.50, 0.05, step=0.01, format="%.2f")
        ml_sell_thr = st.number_input("Sell threshold (e.g., -0.05 = -5%)", -0.50, -0.01, -0.05, step=0.01, format="%.2f")
...
if st.session_state.analysis_run:
    ...
    with tab3:
        ...
        chart_df = yf.download(ticker_for_chart, period="3y", interval="1wk", progress=False, threads=True)
        if not chart_df.empty:
            ...
            if not chart_df.empty:
                ...
                st.caption(f"🌀 Elliott Phase: **{phase_text}** |  Wave#: **{wave_no}** |  ZigZag: {zz_pct*100:.1f}% / {zz_min_bars} weeks")
        else:
